...
if __name__ == '__main__':
    ...
    # debug : 类似debug调试,输出详细的调用过程! 每行添加一个debug输出都不过分!!!
    # 日志中占比最多!!!!
    logger.debug("【调试】进入主程序入口，开始初始化日志")

    # 【info】正常流程日志，记录程序运行状态
    # 记录核心点  入口  +  出口 输出对应的核心参数 可疑点位置输出核心参数
    logger.info("【信息】logger.py内部调用（仅测试，业务模块调用会显示正确文件名）")

    print(f"日志文件输出路径：{LOG_FILE_PATH}")

    # 【warning】警告，不影响运行，但需要关注
    # 有些核心参数不存在,但是不影响业务逻辑! 使用warning   output_dir =   指定生成md pdf 存储的位置
    # if not output_dir :  给与默认值 打印日志 warning  正常执行业务!!!
    logger.warning("【警告】未读取到自定义配置，使用默认配置")

    # 【error】当前功能出错，程序不会崩溃，但业务失败
    # 业务错误 有些核心参数不存在,业务无法继续执行!
    # local_file_path = 每次传入的文件地址 md pdf   后续的业务
    # if  not local_file_path : logger.error   raise 异常...
    # error 已知的异常 if判断中
    ...
    try:
        ...
        # 等待异常 except
        # exception 不仅仅会打印输出内容 也会打印异常的堆栈信息
        # exception ---> except
